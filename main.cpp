...
int main(int argc, char** argv)
{
    ...
    SetupPlayer2Board(&visual_Board, p2_Cards, p2_Cards_Count);
    ...
    SetupPlayer1Board(&visual_Board, p1_Cards, p1_Cards_Count);
    ...
    while (!WindowShouldClose()) 
    {
        ...
        if(selected_Card && isMouseLeftReleased)
        {
            ...
            if(card_Collision_Index >= 0)
            {
                ...
                if(stack_Total <= 10)
                {
                    selected_Card->stack_Value = stack_Total;
                    board_Cards[card_Collision_Index].stack_Value = stack_Total;
                    ...
                    for(smax i = selected_Card->card_Index + 1; i < 4; ++i)
                    {
                        --player_Cards[i].card_Index;
                        //if(i < 4)
                        //    player_Cards[i].stack_Value = player_Cards[i+1].stack_Value;
                    }
            ...
            else if(CheckCollisionRecs(selected_Card->rectangle, visual_Board.play_Zone))
            {
                ...
                board_Cards[board_Cards_Count].collidable = true;
                board_Cards[board_Cards_Count].card_Index = 0;
                board_Cards[board_Cards_Count].stack_Value = selected_Card->stack_Value;
                ++board_Cards_Count;
